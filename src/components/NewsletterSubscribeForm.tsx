...
export default function NewsletterSubscribeForm() {
  ...
  return (
    <form onSubmit={onSubmit} className="flex max-w-full flex-col gap-2.5">
      <input
        type="email"
        name="email"
        autoComplete="email"
        ...
        className="w-full rounded-[10px] border-0 bg-[#2e2e2e] px-[14px] py-3 text-[15px] text-neutral-100 placeholder:text-neutral-500 focus:outline-none focus:ring-2 focus:ring-[#f37121] disabled:opacity-60"
      />
      <button
        type="submit"
        disabled={status === "loading"}
        className="w-full rounded-[10px] bg-[#f37121] px-5 py-3 text-[15px] font-bold uppercase tracking-wide text-neutral-950 hover:bg-[#e55a0a] disabled:opacity-60"
      >
      ...
      {message ? (
        <p
          className={
            status === "error" ? "text-sm text-red-400" : "text-sm text-neutral-400"
          }
